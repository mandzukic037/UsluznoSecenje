...
public class PorudzbineService {
    ...
    private String buildAdminMailText(Porudzbina p) {
        ...
        sb.append("Kupac: ").append(p.getIme()).append(" ").append(p.getPrezime()).append("\n");
        ...
        sb.append("Adresa: ").append(p.getAdresa()).append(", ")
          .append(p.getPostBroj()).append(" ").append(p.getGrad()).append("\n");
        sb.append("\nPlaćanje: ").append(p.getPlacanje()).append("\n");
        sb.append("\nStavke:\n");
        for (PorudzbineStavka s : p.getStavke()) {
            sb.append("  - ").append(s.getNaziv()).append(" x")
              .append(s.getKolicina()).append(" = ")
              .append(s.getCena() * s.getKolicina()).append(" RSD\n");
        }
        sb.append("\nUkupno bez PDV: ").append(p.getUkupnoBezPdv()).append(" RSD\n");
        sb.append("PDV (20%): ").append(p.getPdv()).append(" RSD\n");
        if ("kurirska".equalsIgnoreCase(p.getNacinDostave()))
            sb.append("UKUPNO: ").append(p.getUkupnoSaPdv()).append(" RSD + troškovi dostave\n");
        else
            sb.append("UKUPNO: ").append(p.getUkupnoSaPdv()).append(" RSD\n");
    ...
    private String buildKupacHtml(Porudzbina p) {
        ...
        <body style="margin:0;padding:40px 20px;background:#0F1115;font-family:Arial,sans-serif;">
        <table width="100%%" cellpadding="0" cellspacing="0">
        <tr><td align="center">
        <table width="650" cellpadding="0" cellspacing="0"
        style="background:#1A1F29;border-radius:18px;overflow:hidden;border-top:4px solid #FF6B00;">
        <tr><td align="center" style="padding:40px;background:linear-gradient(180deg,rgb(32,38,49) 0%%,#1A1F29 100%%);">
        <img src="cid:logo" alt="Uslužno sečenje" style="max-height:90px;margin-bottom:20px;">
        <div style="color:#FF6B00;font-size:11px;letter-spacing:3px;text-transform:uppercase;font-weight:bold;">
        POTVRDA PORUDŽBINE</div>
        <h1 style="color:#F3F4F6;margin:15px 0 0;font-size:34px;">Hvala na porudžbini!</h1>
        </td></tr>
        <tr><td style="padding:40px;">
        <p style="color:#F3F4F6;font-size:16px;">Poštovani <strong>%s</strong>,</p>
        <p style="color:#B5BBC5;font-size:15px;line-height:1.7;">
        Vaša porudžbina <strong>#%d</strong> je uspešno primljena i trenutno je u obradi.</p>
        <div style="margin:30px 0;padding:25px;border-radius:12px;background:#202631;border:1px solid rgba(255,107,0,0.25);">
        <div style="color:#FF6B00;font-size:12px;text-transform:uppercase;letter-spacing:2px;margin-bottom:10px;">Ukupan iznos</div>
        <div style="color:#F3F4F6;font-size:28px;font-weight:bold;">%s</div>
        </div>
        <p style="color:#B5BBC5;font-size:15px;line-height:1.7;">Kontaktiraćemo vas u najkraćem roku radi potvrde porudžbine.</p>
        </td></tr>
        <tr><td style="padding:25px;background:#141922;border-top:1px solid rgba(255,255,255,0.05);">
        <p style="margin:0;color:#8C95A3;text-align:center;font-size:13px;">
        Uslužno sečenje • Profesionalno CNC i lasersko sečenje</p>
        </td></tr>
        </table></td></tr></table>
        </body></html>
        """.formatted(p.getIme(), p.getId(), ukupno);
    }
